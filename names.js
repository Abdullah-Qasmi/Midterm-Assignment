...
// Browser localStorage se favorites load karo

let favorites =
    JSON.parse(localStorage.getItem("allahFavorites")) || [];
...
function toggleFavorite(id) {
    ...
    if (showingFavorites) {
        ...
        const favoriteNames =
            ALLAH_NAMES.filter(
                name => favorites.includes(name.id)
            );

        renderNames(favoriteNames);
...
function renderNames(filteredNames) {
    ...
    filteredNames.forEach(name => {
        ...
        card.className =
            "bg-surface-container-lowest border border-outline-variant rounded-2xl p-6 hover:shadow-lg hover:border-primary-container transition-all cursor-pointer group flex flex-col justify-between h-48 relative overflow-hidden";
        ...
        card.innerHTML = `
            ...
            <div class="flex justify-between items-start relative z-10">
                ...
                <span
                    class="text-xs font-bold text-primary-container bg-primary-fixed-dim/20 px-2 py-1 rounded-md">
            ...
            <div
                ...
                <h3
                    class="arabic-text text-3xl text-on-surface mb-1 group-hover:scale-110 transition-transform">
            ...
            <div
                ...
                <p
                    class="text-xs text-outline font-medium line-clamp-1">
...
function openModal(name) {
    ...
    document.getElementById("modalUrdu")
        .textContent =
        name.urdu;
...
if (allTab) {
    ...
    allTab.addEventListener(
        ...
        function () {
            ...
            // Inactive Favorites tab

            favoriteTab.classList.remove(
                "border-primary-container",
                "text-on-surface"
            );

            favoriteTab.classList.add(
                "border-transparent",
                "text-outline"
            );
...
if (headerFavoriteButton) {
    ...
    headerFavoriteButton.addEventListener(
        "click",
        function () {
